const chalk_global = require('chalk');
const Slack = require('slack-node');
const apiToken = "xoxb-112438842800-U4g07vrFe0DzpiDJywZs9w1z";
...
const SlackReporter = function(baseReporterDecorator, config) {
    ...
    var chalk = new chalk_global.constructor({
        enabled: config.colors
    });
    ...
    this.onRunComplete = function(browsers, results) {
        ...
        if (counts.hidden) {
            slack.api('chat.postMessage', {
                text: "  " + chalk.green('' + counts.hidden) +
                    (counts.shown ? " more" : "") +
                    " test cases successful in all browsers\n",
                channel: '#visumservice-tests'
